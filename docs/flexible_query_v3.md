...
This query is an upgrade from v2. It is now able to handle multiple sensors. Moreover, it can accept
an aggregation function (`AVG`, `MIN`, `MAX`, `STDDEV`) from a dropdown.

## Create variable for aggregation function

Go to `Dashboard settings` -> `Variables` -> `Add variable` and use the following settings:

- **Variable type**: `Custom`
- **Name**: `func`
- **Values separated by comma**: `MIN,MAX,AVG,STDDEV`

```sql
DROP PROCEDURE IF EXISTS get_multiple_sensors_for_multiple_axes_multiple_machines_v3;
DELIMITER //
CREATE PROCEDURE get_multiple_sensors_for_multiple_axes_multiple_machines_v3(
    /* Set parameters */
    IN axes JSON,
    IN sensors JSON,
    IN machines TEXT,
...
Just like before, you need to create one of these for each pair of axes that you have.

```sql
CALL get_multiple_sensors_for_multiple_axes_multiple_machines_v3(
