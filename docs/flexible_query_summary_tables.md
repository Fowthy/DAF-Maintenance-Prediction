## Flexible query for summary tables

This query gets sensor data from the summary tables (for example: AVG_SUMMARY_A11_Axis_A1_Axis)
It is very similar to Flexible Query v3, however it iterates over the summary tables.

```sql
DROP PROCEDURE IF EXISTS flexible_query_from_summary_table;
DELIMITER //
CREATE PROCEDURE flexible_query_from_summary_table(
    /* Set parameters */
    IN axes JSON,
    IN sensors JSON,
    IN machines TEXT,
...
BEGIN
    ...
    */
    ...
    /* summary table name template avg_summary_x_y */


    /* Find table name for the current axis. For example `AVG_SUMMARY_A11_A1`. */
    ...
    /* func is either AVG,MIN,MAX,STDEV and the data is gathered from the summary tables*/
    SET @twin_axis = REGEXP_REPLACE(@axis_table_name, CONCAT(func,"_SUMMARY_(.+)_Axis_(.+)_Axis"), "$1");
    IF @twin_axis = current_axis THEN
        SET @twin_axis = REGEXP_REPLACE(@axis_table_name, CONCAT(func,"_SUMMARY_(.+)_Axis_(.+)_Axis"), "$2");
    END IF;
            ...
            WHILE j < JSON_LENGTH(sensors)
                DO
                    ...
                    SET main_union_selections = CONCAT(
                            ...
                            "' as 'As', machine, ", REPLACE(axis, '"', ''), '_', REPLACE(sensor, '"', ''),
...
You call the query in the following format:

```sql
CALL flexible_query_from_summary_table(
  '["Axis1,Axis2"]', -- Single quotes are important.
  '["Sensor1, Sensor2"]',
  "3,4,5", -- bfc_source_id
  "func", -- either MIN,MAX,AVG,STDEV
...
```
And a complete example looks like this:
(This is called directly from MySQL Workbench)
```sql
CALL flexible_query_from_summary_table(
  '["A11"]', -- Single quotes are important.
  '["Position"]',
  "3",
  "AVG",
  "UNIX_TIMESTAMP(data.BFC_TIMESTAMP) DIV 900 * 900 as time",
...
To call it from Grafana:
(It gets the data from variables axis, sensors, machines, fun and the datetime range)
```sql
