...
The difference of this version from v3 is that we get rid of the `INNNER JOIN`
that happens on every query and we delegate that responsibility of showing the
`BFC_CLIENT_ID`(`M22838`, etc..) to _Grafana_.

```diff
FROM
-  DATASET_C_Axis_B_Axis as data INNER JOIN sources on data.BFC_SOURCE_ID = sources.ID
+  DATASET_C_Axis_B_Axis as data
```

## Query

```sql
DROP PROCEDURE IF EXISTS get_multiple_sensors_for_multiple_axes_multiple_machines_v4;
DELIMITER //
CREATE PROCEDURE get_multiple_sensors_for_multiple_axes_multiple_machines_v4(
    /* Set parameters */
    IN axes JSON,
    IN sensors JSON,
    IN machines TEXT,
...
> Note: I have attached an export of a Grafana Dashboard with those things setup
> already.

Just like before, you need to create one of these for each pair of axes that you
have.

```sql
CALL get_multiple_sensors_for_multiple_axes_multiple_machines_v4(
...
Go `Transform` and add a `Rename by regex` for each machine that you have. For
example,
